...
raw_train = pd.read_csv('./data/train.csv', nrows=20000, skiprows=0)
...
for f in ['event_type', 'category_code', 'brand']:
    # 构建编码器
    le = LabelEncoder()  # LabelEncoder就是把n个类别值编码为0~n-1之间的整数，建立起1-1映射  进行独热编码
    le.fit(df[f])
...
for uid in tqdm(user_ids):
    ...
    # 滑动窗口，步长为1--后一个时间节点的商品ID，将product_id列上移一列将其并入y作为输出结果
    user_data['y'] = np.nan
